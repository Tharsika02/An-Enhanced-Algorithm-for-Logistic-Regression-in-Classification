...
df1 <- read.csv("oil_spill_csv.csv")
df <- na.omit(df1)
dim(df)
...
model1_GLM <- glm(class ~ . - 1, data = train_data, family = "binomial")
coefficients <- coef(model1_GLM)
#summary(model)

#predicting target using logistic regression
predicted_class <- predict(model1_GLM, newdata = test_data, type = "response")
